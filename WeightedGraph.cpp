...
*******************************************************************
Laboratory 12
Weighted Graph Lab || Group Project               WeightedGraph.cpp
...
// vertices (defaults to defMaxGraphSize).
  ...
  : maxSize(maxNumber),
    size(0)
{
    vertexList = new Vertex [ maxSize ];  // <-------- fill in the member variable name

    adjMatrix = new int [ maxSize*maxSize ];  // <---- fill in the member variable name
...

{
	...
	setEdge(idx_v1,idx_v2,wt);  // <-------- fill in the helper function name
    setEdge(idx_v2,idx_v1,wt);  // <-------- fill in the helper function name
}
...
{
    int idx_v1 = getIndex(v1) ;   // Index of vertex v1    // <----- complete this line
    int idx_v2 = getIndex(v2) ;   // Index of vertex v2    // <----- complete this line
    ...
    wt = getEdge(idx_v1,idx_v2);  // Get weight of edge using helper function  <- complete this line
...
{
    return adjMatrix[row * maxSize + col];  // <-- fill in the array index
}
